#' @useDynLib SA23204165
#' @import MASS
#' @import stats
#' @importFrom graphics plot lines legend 
#' @title  plot fitting path
#' @description A Gibbs sampler using R
#' @param fit return of g.LARS function 
#' @param xrange xlabel range
#' @param yrange ylabel range
#' @return a list with generated coefficient path and add path 
#' @examples
#' \dontrun{
#' library(SA23204165)
#' library(MASS)
#' rho = 0.2
#' X <- mvrnorm(200,mu = rep(0,10), Sigma = diag(10) + rho*(1-diag(10)))
#' beta <- c(0.2,0.4,0.5,0.6,0.6,0.6,0.2,0.3,0.5,0.7)
#' g.index <- c(1,1,1,2,2,2,3,3,4,5)
#' y <- X%*%beta + rnorm(200)
#' fit <- glars(X,y,g.index)
#' path(fit)
#' }
#' @export
path <- function(fit,xrange = c(0,1), yrange = c(0,1)) {
  betas = fit$betas
  g.index = fit$index
  K = fit$K
  grps = unique(g.index)
  group.norm <- function(beta){  #一列值
    group.norm.f <- function(j){ #第j组的norm
      my.norm(beta[g.index == j])
    }
    sapply(grps,group.norm.f)
  }

    betas.diff = matrix(0,nrow(betas),ncol(betas))
    betas.diff[,1] = betas[,1]
    for (i in 2:ncol(betas)){
      betas.diff[,i] = betas[,i] - betas[,i-1]
    }
    points = numeric(ncol(betas)+1)
    for (k in 1:ncol(betas)){
      points[k+1]= sum(colSums(apply(betas.diff,2,group.norm)*sqrt(K))[1:k]) / sum(colSums(apply(betas.diff,2,group.norm)*sqrt(K)))
    }  
  plot(0,0,type = "l",xlim = xrange,ylim = yrange,xlab = "fraction",ylab = "beta",main = 'gLARS')
  for(i in 1:nrow(betas)){
    lines(points,c(0,betas[i,]),type = "l",col = i,lty = i,lwd = 2)
  }
  legend("topleft",paste("beta",1:nrow(betas)),col = 1:nrow(betas),lty = 1:nrow(betas),cex = 0.8)
}

my.norm <- function(x){
  sqrt(sum(x^2))
}